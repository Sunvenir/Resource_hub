...
public interface ExpertMapper {
        @Insert({ "insert into Expert(expertID,expertName,institution,brief,field,avatarUrl) values(#{expertID},#{expertName},#{institution},#{brief},#{technicalField},#{avatarUrl})" })
        int insertExpert(Expert expert);

        @Update({ "update Expert set institution=#{institution},brief=#{brief},field=#{technicalField} where expertID=#{expertID}" })
